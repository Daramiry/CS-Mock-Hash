...
checkPassword = async (username, plaintextPassword) => {
    ...
    if (globalStore[username]) {
        ...
        if (result) {
            ...
            console.log('Congratulation, you have logged in. want to play globalthermonucear war')
        }
        else {
            // TODO: Display message for invalid credentials
            console.log('Get the heck out of here you hecken hacker')
        }
    }
    else {
...
hashPassword = async (username, password) => {
    ...
    console.log(`Added password hash to globale store `)
}
